...
class ContinuousLearner:
    def __init__(self, mongo_uri: str):
        ...
        self.feature_weights = {
            'location': 1.0,
            'remote_work': 0.8,  # New weight for remote work compatibility
            'experience': 1.0,
            'skills': 1.0,
            'resume_content': 1.0
        }
    ...
    def _extract_features(self, candidate: Dict[str, Any], vacancy: Dict[str, Any]) -> Dict[str, float]:
        ...
        if candidate.get('location') and vacancy.get('location'):
            # Check if the vacancy allows remote work
            if vacancy.get('remote_option', False):
                # If remote work is allowed, location match gets full score
                features['location'] = 1.0
                # Add remote work compatibility score
                features['remote_work'] = 1.0
            else:
                # Traditional location matching for non-remote positions
                features['location'] = 1.0 if candidate['location'].lower() == vacancy['location'].lower() else 0.0
                features['remote_work'] = 0.0
        else:
            features['location'] = 0.0
            features['remote_work'] = 0.0
